...
def scan_ds18(force_log=False):
    ...
    try:
        ...
        if force_log or ds18_roms:
            log("DS18B20 devices on GPIO%d: %d" % (DS18_PIN, len(ds18_roms)))
    except:
        ds18_roms = []
        ds18_last_scan_ms = time.ticks_ms()
        if force_log:
            log("DS18B20 scan failed on GPIO%d" % DS18_PIN)
...
def sensor_loop_step():
    global ds18_last_read_ms, ds18_roms, dht_temp, dht_hum, door_state, door2_state, di_states, last_sensor_poll_ms
    ...
    # DS18B20 read on GPIO42 (1-Wire)
    if (not ds18_roms) and time.ticks_diff(now, ds18_last_scan_ms) >= DS18_SCAN_MS:
        scan_ds18()

    if ds18_roms and (time.ticks_diff(now, ds18_last_read_ms) >= DS18_READ_MS or dht_temp is None):
        try:
            ds18.convert_temp()
            time.sleep_ms(750)
            new_temp = ds18.read_temp(ds18_roms[0])
            ds18_last_read_ms = now

            if new_temp is None or not (-55.0 <= new_temp <= 125.0):
                return
...
def html_page():
    ...
    html = (
        ...
        "<div class='card'>"
        "<h3>Sensors</h3>"
        "<div class='line'>DS18B20 GPIO42: <b id='tempVal'>--</b> C</div>"
        "<div class='line'>DS18B20 devices: <b id='ds18CountVal'>--</b></div>"
...
ds18 = ds18x20.DS18X20(onewire.OneWire(Pin(DS18_PIN)))
