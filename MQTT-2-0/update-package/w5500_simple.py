...
class W5500:
    ...
    Sn_DIPR = 0x000C # Destination IP Address (4 bytes)
    Sn_DPORT = 0x0010 # Destination Port (2 bytes)
    ...
    def socket_connect(self, sock, ip, port):
        """Connect socket to remote host"""
        block = self.socket_block(sock)
        self.write_bytes(self.Sn_DIPR, ip, block)
        self.write_reg16(self.Sn_DPORT, port, block)
        self.write_reg(self.Sn_CR, self.SOCK_CONNECT, block)
        while self.read_reg(self.Sn_CR, block) != 0:
            pass
        return True
