...
print(f"Button on D12 (GPIO {BUTTON_PIN})")
...
def html_page():
    ...
    door_str = "DOOR CLOSED" if door_val else "DOOR OPEN"
    door_color = "#f44336" if door_val else "#4CAF50"
...
def handle_request(req):
    ...
    if b'GET /p?' in req:
        ...
        door = door_pin.value()
        btn = button_pin.value()
        json = '{"t":%s,"h":%s,"r":[%s],"d":%d,"b":%d}' % (
            t if t else 'null',
            h if h else 'null',
